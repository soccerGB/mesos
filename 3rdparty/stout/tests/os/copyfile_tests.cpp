...
{
  const string sandbox = os::getcwd();
  const string fileContents = "Some file content";

  const string sourceFile = path::join(sandbox, "source-file");
  EXPECT_SOME(os::write(sourceFile, fileContents));

  const string destination = path::join(sandbox, "dest-file");
...
TEST_F(CopyFileTest, CopyToDirectoryDestinationFails)
{
  ...
  const string destination = path::join(sandbox, "dest-dir");
  EXPECT_FALSE(os::exists(destination));
