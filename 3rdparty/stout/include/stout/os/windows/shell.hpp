...
//
// The caller can specify explicit`stdin`, `stdout`, and `stderr` handles for
// the process; however, it must be all or none. This function will return an
// error if only a subset are set.
//
...
inline Try<ProcessData> create_process(
    ...
    const Option<int_fd>& stdin_fd = None(),
    const Option<int_fd>& stdout_fd = None(),
    const Option<int_fd>& stderr_fd = None())
{
  ...
  if (stdin_fd.isSome() && stdout_fd.isSome() && stderr_fd.isSome()) {
    startup_info.hStdInput = stdin_fd.get();
    startup_info.hStdOutput = stdout_fd.get();
    startup_info.hStdError = stderr_fd.get();
    startup_info.dwFlags |= STARTF_USESTDHANDLES;
  } else if (stdin_fd.isSome() || stdout_fd.isSome() || stderr_fd.isSome()) {
    return Error("A handle was given, but not every handle was given.");
  }
