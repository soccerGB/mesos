    ...
    const std::string& path = path::join(os::temp(), "XXXXXX"))
{
  ...
  const std::string temp = stringify(std::wstring(buffer.data()));
  ...
  Try<int_fd> fd = os::open(temp, _O_CREAT, _S_IREAD | _S_IWRITE);
  if (fd.isError()) {
  ...
  return strings::remove(temp, os::LONGPATH_PREFIX, strings::Mode::PREFIX);
}
