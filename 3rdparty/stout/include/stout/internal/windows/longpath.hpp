...
// This function idempotently prepends "\\?\" to the given path if it needs it:
// (1) the path's length is greater than 248, the minimum Windows API limit
//     (this limit != NAME_MAX nor PATH_MAX, it is an arbitrary limit of
//     `CreateDirectory`, and is the smallest such limit)
// (2) the path is absolute (otherwise the marker is meaningless)
// (3) the path does not already have the marker (idempotent)
//
// It then converts it to UTF-16, appropriate for use in Uniode versions of
// Windows filesystem APIs which support lengths greater than NAME_MAX.
