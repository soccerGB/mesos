...
TEST_F(FetcherTest, FileURI)
{
  ...
  uri->set_value(uri::uriFromFilename(testFile, uri::Scheme::FILE));
...
TEST_F(FetcherTest, AbsoluteFilePath)
{
  ...
  CommandInfo commandInfo;
  CommandInfo::URI* uri = commandInfo.add_uris();
  uri->set_value(uri::uriFromFilename(testPath, uri::Scheme::FILE));
...
// Also, permissions handling needs to be worked out.
...
TEST_F(FetcherTest, ExtractTar)
{
  ...
  CommandInfo commandInfo;
  CommandInfo::URI* uri = commandInfo.add_uris();
  uri->set_value(uri::uriFromFilename(path.get() + ".tar", uri::Scheme::FILE));
  uri->set_extract(true);
...
TEST_F(FetcherTest, Unzip_ExtractFile)
{
  ...
  CommandInfo commandInfo;
  CommandInfo::URI* uri = commandInfo.add_uris();
  uri->set_value(uri::uriFromFilename(path.get() + ".zip", uri::Scheme::FILE));
  uri->set_extract(true);
...
TEST_F(FetcherTest, UseCustomOutputFile)
{
  ...
  CommandInfo commandInfo;
  CommandInfo::URI* uri = commandInfo.add_uris();
  uri->set_value(uri::uriFromFilename(path.get(), uri::Scheme::FILE));
  uri->set_extract(true);
  uri->set_output_file(customOutputFile);
